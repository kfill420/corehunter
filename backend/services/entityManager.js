...
class EntityManager {
    ...
    init() {
        ...
        // Boucle à 30 FPS (delta environ 33ms)
    ...
    update(delta) {
        ...
        this.io.emit("slimeUpdate", this.slimes);
    }

    // Appelé par socketManager quand un joueur bouge
    updatePlayerPos(id, data) {
    ...
    handleHitSlime(data) {
        ...
        if (slime && !slime.dead) {
            slime.hp -= damage;
            console.log(`Slime ${id} a reçu ${damage} dégâts. HP restants: ${slime.hp}`);

            if (slime.hp <= 0) {
                slime.dead = true;

                this.io.emit("slimeStatUpdate", { id: slime.id, dead: true });

                setTimeout(() => {
                    delete this.slimes[id];
                }, 3000);
            } else {
                this.io.emit("slimeStatUpdate", { id: slime.id, hp: slime.hp, dead: false });
            }
        }
    }
}
